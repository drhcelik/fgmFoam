...
defineTypeNameAndDebug(PVtableSolver, 0);
defineRunTimeSelectionTable(PVtableSolver, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

PVtableSolver::PVtableSolver(const fvMesh& mesh, const wordList& tableNames)
:
   IOdictionary
   (
      IOobject
   ...
   tables_(tableNames_.size() + 1)
{
	...
	tableNames_[i] = tableNames_[i] + "_table";
        ...
        tables_.set(i, PVTable::New(mesh, tablePath_,  tableNames_[i]));
    }

    paramNames_[0] = "varZ_param";
...
PVtableSolver::~PVtableSolver()
{}

// * * * * * * * * * * * * * *  Member Functions * * * * * * * * * * * * * * //

List<int> PVtableSolver::upperBounds(const scalarList& x) const
{
...
scalarList PVtableSolver::position(const List<int>& ub, const scalarList& x) const
{
...
scalar PVtableSolver::interpolate(const List<int>& ub , const scalarList& pos, const label& i) const
{
    return tables_[i].PVinterpolate(ub, pos);
}

int PVtableSolver::sizeTableNames() const
{
